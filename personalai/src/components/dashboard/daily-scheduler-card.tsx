...
export default function RoutineGenerator() {
  ...
  const moods = Object.keys(moodEmojiMap) as (keyof typeof moodEmojiMap)[];

  const [mood, setMood] = useState<keyof typeof moodEmojiMap>("Happy");
      ...
      >
        <div className="mb-8 text-center">
          ...
          <p className="text-muted-foreground">
            Personalized schedule based on your mood and commitments
          </p>
        </div>
        ...
        <Card className="modern-card mb-8">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="inline-flex items-center border rounded-md border-primary/30 text-lg px-3 py-1">
                <span className="mr-2">{moodEmojiMap[mood] || "🙂"}</span>
                <label className="mr-2">Feeling</label>
                <select
                  value={mood}
                  onChange={(e) => setMood(e.target.value as keyof typeof moodEmojiMap)}
              ...
              <Card className="modern-card overflow-hidden">
                ...
                <CardFooter className="flex justify-center border-t bg-muted/20 py-3">
                  <p className="text-xs text-muted-foreground">
                    This routine is designed to help manage your{" "}
                    {mood.toLowerCase()} feelings
                  </p>
                </CardFooter>
              </Card>
            </motion.div>
